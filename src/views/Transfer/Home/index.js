...
function Transfer() {
  ...
  const getListUser = async (token) => {
    try {
      const listAllUser = await http(token).get("/user?limit=4");
  ...
  const searchUser = async (event) => {
    try {
      const listAllUser = await http(user.token).get(
    ...
    <>
      <div className="card px-4 pt-3 border-rad shadow-sm">
        <div className="card-header bg-transparent border-0">
          <div className="row">
            <p className="fw-bold text-dark">Search Receiver</p>
          </div>
          <div className="row">
            <div className="position-relative d-flex align-items-center">
              ...
              <input
                type="text"
                className="form-control py-2 ps-5 border-0 border-rad_input"
                placeholder="Search receiver here"
                aria-label="Username"
                aria-describedby="addon-wrapping"
                style={{ backgroundColor: "rgba(58, 61, 66, 0.1)" }}
                name="search"
              ...
              {isLoading === true ? (
                ...
                listUser.map((user, index) => {
                  return (
                    <>
                      <Link
                        to={`/transfer/input-amount/${user.id}`}
          ...
          <nav aria-label="Page navigation example">
            <ul className="pagination justify-content-center">
              <li className="page-item">
                <button
                  className={`page-link btn bg-transparent ${
                    isLoading === true
                      ? ""
                      : `${link.prevLink === null ? "disabled" : ""}`
                  }`}
                  href="#"
                  tabIndex="-1"
                  name="prev"
                  onClick={(event) => handleClickList(event)}
                >
              ...
              <li className="page-item">
                <span className="page-link" href="#">{`${
                  isLoading === true ? "" : `${link.currentPage}`
                }`}</span>
              </li>
              <li className="page-item">
                <button
